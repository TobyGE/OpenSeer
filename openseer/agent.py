...
## Tools
  ...
  read_page      READ a webpage's content (posts/articles/search results/threads). Uses the user's browser via AppleScript JS injection by default. Optional `url` (navigate first, in the same call), `app` (Safari/Chrome/Arc/Brave/Edge — default = frontmost), `selector` (CSS like `"article"`/`"main"` for one section only). Use this for ANY task that says "summarize / read / browse / find X in posts / what does this page say". Requires the one-time "Allow JavaScript from Apple Events" toggle in the browser menu. CDP article extraction / XHR capture exists but is disabled unless `OPENSEER_BROWSER_CDP=auto` or `on` is explicitly set.
  read_pages     READ multiple webpages in ONE call. CDP-only and disabled by default; prefer sequential `read_page` unless `OPENSEER_BROWSER_CDP=auto` or `on` is explicitly set.
  save_pdf       SAVE a webpage to local PDF via Chrome's Page.printToPDF. CDP-only and disabled by default; requires `OPENSEER_BROWSER_CDP=auto` or `on` before use.
  click          `ax_query` (preferred — natural-language label match against the AX tree, e.g. `ax_query="Sign in"` or `ax_query="button: Submit"`) OR `index` (AX-tree row number — use when ax_query is ambiguous and you've eyeballed the right row) OR `x`, `y`. Optional `count`.
  type           `text` + one of: `ax_query` (preferred), `index`, `x,y`, or NEITHER (use currently-focused field).
    ...
    for i in range(max_steps):
        ...
        if ask_user is not None:
            instructions = instructions + "\n\n" + (
                ...
                "`ask_user` is wired this run. **Prefer ask_user over "
                "terminate(fail) whenever you'd otherwise stop to wait "
                "for user input.** The two have different meanings:\n\n"
                "  - `ask_user` = pause this task for a quick answer, "
                "    then resume from the user's reply on the next turn. "
                "    The model sees `User reply (kind=...): <answer>`.\n"
                "  - `terminate(fail)` = the task is over, I'm stuck and "
                "    can't continue. The user has to send a new message "
                "    to make anything happen. Reserve this for genuine "
                "    blockers (network down, app missing, contradictory "
                "    state). DO NOT use it as a way to ask a question.\n\n"
                "Decision flow at every choice point:\n\n"
                "  1. **Look in MEMORY.md first.** If the answer is there "
                "     (payment default, shipping address, preference), USE "
                "     IT. Don't ask the user the same question twice.\n"
                "  2. **Confidence high, action reversible** → just act. "
                "     No need to ask.\n"
                "  3. **Confidence high, action hard-to-reverse** "
                "     (pay / send / post / delete / overwrite) → "
                "     ALWAYS confirm with `ask_user(kind=\"confirm\")` "
                "     and attach a screenshot of the current screen so "
                "     the user can see what's about to happen.\n"
                "  4. **Multiple plausible options, no MEMORY.md hit** "
                "     (which card / seat / address / file / showtime) → "
                "     `ask_user(kind=\"choose\", options=[...])` with the "
                "     viable options. List the most likely options as "
                "     buttons.\n"
                "  5. **Need free-form info** (custom note, address line "
                "     when none cached, the title of a thing) → "
                "     `ask_user(kind=\"text\")`.\n\n"
                "Common temptations to RESIST:\n"
                "  - \"User said pick rear-center seats; let me just click "
                "    them.\" — picking a *specific* seat (G6 vs G7 vs G8) "
                "    when the user only specified a *region* is a rule-4 "
                "    choose-ask, not a free decision. Show the seat map "
                "    via attachments and offer the candidates.\n"
                "  - \"I reached the checkout page; let me terminate(fail) "
                "    and tell the user to fill payment.\" — that drops the "
                "    task. Use `ask_user(kind=\"text\", question=\"Confirm "
                "    purchase? Need email + card to fill.\")` with a "
                "    screenshot, OR if MEMORY.md has the email and you "
                "    just need confirmation, `ask_user(kind=\"confirm\")`.\n"
                "  - \"I'm uncertain; safer to stop.\" — uncertainty is "
                "    exactly what ask_user is for. Surface the question, "
                "    don't bail.\n\n"
                "After getting a reply, when the answer is durable user "
                "info that should persist for FUTURE runs (\"my AMEX "
                "1234 is the default\", \"my address is X\", \"I prefer "
                "aisle seats\"), emit `write_memory` with a one-line "
                "bullet so we don't have to ask again. Never write task-"
                "specific values (the title they asked you to look up, "
                "a one-time URL).\n\n"
                ...
                "show the user along with the question — strongly recommended "
                "for confirm/choose so the user can see what's about to "
                "happen. To capture the current screen first: "
                "`bash screencapture -x /tmp/openseer-confirm.png` then list "
                "that path in attachments.\n\n"
